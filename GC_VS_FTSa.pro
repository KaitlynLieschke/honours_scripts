...
;
; PURPOSE:
;       compare Bomem FTS data to model output, specifically Wollongong vs
...
;       AVGKERNEL: file containing averaging kernel of the instrument
;       (path from kjl574/)
;
; OUTPUT:
;       plot of GEOS_Chem output vs measurements
;
; SUBROUTINES:
...
pro GC_VS_FTSa, year=year, diag=diag, trac=trac

; Select fts file and path
ftsfile = '/short/m19/kjl574/sfit/fts_co_woll_'+year+'a.hdf'
...
; READ IN MODEL DATA
;===============================================================
; Create variable to count fts measurements that are saved
count = 0
...
; If first fts value is earlier than first model value, go to next fts_value
...
; If all tau values for the month have been meatched then break
...
fts_pres = data_pr[*,init:(init+nsets-1)]
fts_ak = data_ak[*,init:(init+nsets-1)]
fts_apri = data_ap[*,init:(init+nsets-1)]
...
write_csv,spec+'_'+year+'_gc_ftsa_data.csv',date_final[0:n_elements(gc_final)-1],gc_final,fts_final,sys_final,ran_final,header = hdr
